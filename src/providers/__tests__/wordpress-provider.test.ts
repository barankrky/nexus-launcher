...
describe('WordPressProvider Integration Tests', () => {
	...
	beforeAll(() => {
		...
		console.log('Provider Configuration:', JSON.stringify(OYUNINDIR_CONFIG, null, 2));
	...
	describe('Test 2: Fetching Games', () => {
		it('should fetch first 5 games with correct structure', async () => {
			...
			games.forEach((game, index) => {
				...
				console.log(`  Description: ${game.description.length > 0 ? '✓' : '✗'} (${game.description.length} chars)`);
				console.log(`  Excerpt: ${game.excerpt.length > 0 ? '✓' : '✗'} (${game.excerpt.length} chars)`);
	...
	describe('Test 3: Fetching Game by ID', () => {
		it('should fetch game by ID with full details', async () => {
			...
			console.log(`    Avatar (24px): ${game!.author.avatarUrls.size24 ? '✓' : '✗'}`);
			console.log(`    Avatar (48px): ${game!.author.avatarUrls.size48 ? '✓' : '✗'}`);
			console.log(`    Avatar (96px): ${game!.author.avatarUrls.size96 ? '✓' : '✗'}`);
	...
	describe('Test 4: Search Functionality', () => {
		it('should search for games and return matching results', async () => {
			...
			if (results.length > 0) {
				...
				results.forEach((game, index) => {
					...
					const descMatch = game.description.toLowerCase().includes(searchQuery.toLowerCase());
					console.log(`     Match: ${titleMatch ? 'Title' : descMatch ? 'Description' : 'None'}`);
				});
		...
		it('should handle empty search query gracefully', async () => {
			...
			console.log('✓ Empty search query handled correctly (returned empty array)');
		});
	});
	...
	describe('Test 5: Pagination Metadata', () => {
		...
		it('should handle different page sizes', async () => {
			...
			for (const size of pageSizes) {
				...
				console.log(`  ✓ Fetched ${paginated.data.length} games with limit ${size}`);
			}
	...
	describe('Test 6: Caching Functionality', () => {
		it('should cache results and improve performance on second call', async () => {
			...
			console.log(`  Speedup: ${((time1 - time2) / time1 * 100).toFixed(2)}%`);
	...
	describe('Test 7: HTML Parsing', () => {
		it('should extract download links from HTML content', async () => {
			...
			if (game!.downloadLinks.length > 0) {
				...
				// Count link types
				const typeCounts = game!.downloadLinks.reduce((acc, link) => {
		...
		it('should sanitize HTML content correctly', async () => {
			...
			console.log('\nSanitized Content:');
			console.log(`  Description length: ${game!.description.length} characters`);
	...
	describe('Test 8: Categories', () => {
		it('should fetch available categories', async () => {
			...
			console.log(`Fetched ${categories.length} categories in ${endTime - startTime}ms`);
